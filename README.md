...
- **2026-03-28 更新：转录清理与界面体验优化**
  - 重构了转录 cleanup 流程，降低"问句被润色成回答"的概率；当模型输出回答式内容时，不再直接退回接近原始 ASR 的保守结果，而是先做一次 cleanup-only retry，再决定是否 fallback。
  - 调整 strict mode 放行逻辑：对于"只删除口吃、重启、填充词"的清理结果，不再因为文本变短就误判为偏离原文。
  - 优化中文口吃/重复处理，重点覆盖短问句、重新起头、自我修正等场景，减少"看起来像没润色"的情况。
  - 优化中文数字规范化规则，改善序数词、口语"一点 / 一下 / 一些"和时间表达式的处理，避免 `第1个`、`干净1点`、`十2.27分` 这类不自然结果。
  - 历史记录改为分页加载，避免旧转录被固定条数截断后误以为"只保留近几天"。
  - 调整悬浮球录音态视觉：将涟漪层移到按钮外层容器，扩散范围从轻微外溢改为更完整的向外传播，并把双层波纹改为三层等间隔波纹，连续感更强。
  - 修复 macOS 下悬浮球在透明窗口中出现磨砂矩形边框的问题：移除了导致整块窗口被渲染为玻璃方框的 blur 方案，并将环境光晕改为有明确透明边界的径向渐变，避免录音时光晕蔓延到整个窗口边缘。
  - 修复涟漪动画在窗口边缘被硬裁切的问题：让波纹在接近窗口边缘前提前淡出，观感更自然。
  - 修复鼠标悬停在悬浮球上时，快捷键提示更新不够实时的问题。
  - 同步补充了相关回归测试，并将 PromptStudio 自定义 prompt 更明确地接入正式 cleanup 流程。
