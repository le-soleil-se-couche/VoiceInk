...
const NON_FATAL_UPDATE_ERROR_CODES = new Set([
...
class UpdateManager {
  ...
  setupAutoUpdater() {
    ...
    // Use the packaged app-update.yml generated by electron-builder as the
    // single source of truth for release ownership. This prevents the fork
    // from accidentally following upstream OpenWhispr releases at runtime.
  ...
  isMissingReleaseError(error) {
    const code = String(error?.code || "").trim();
    if (NON_FATAL_UPDATE_ERROR_CODES.has(code)) {
      return true;
    }
  ...
  buildNoUpdateResult(message = "No published VoiceInk updates yet") {
    this.updateAvailable = false;
    this.updateDownloaded = false;
    this.isDownloading = false;
    this.lastUpdateInfo = null;
    this.notifyRenderers("update-not-available", { message });
    return {
  ...
  async checkForUpdates() {
    try {
      ...
      if (this.isMissingReleaseError(error)) {
        console.log("ℹ️ No published VoiceInk updates found yet");
        return this.buildNoUpdateResult();
      }
