...
export async function initializeTranscriptions(limit = DEFAULT_LIMIT) {
  ...
  const savedState = loadPaginationState();
  ...
  const nextState = {
    ...
    hasMore: savedState?.hasMore ?? (items.length === limit),
    isLoadingMore: false,
    oldestLoadedId: savedState?.oldestLoadedId ?? (items.length > 0 ? items[items.length - 1].id : null),
  };
  useTranscriptionStore.setState(nextState);
  // Persist pagination state to localStorage
  savePaginationState({ hasMore: nextState.hasMore, oldestLoadedId: nextState.oldestLoadedId });
  return items;
}
...
export function clearTranscriptions() {
  ...
  if (typeof window !== "undefined" && window.localStorage) {
    try {
      window.localStorage.removeItem(PAGINATION_STORAGE_KEY);
    } catch (e) {
