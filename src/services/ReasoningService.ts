...
- If the source is short/ambiguous, only minimally clean it; do not transform it into assistant dialogue.`;
  ...
  private localCleanupFallback(text: string): string {
    return text
      ...
      .replace(/([，,、])\s*(?:那个|就是)\s*[，,、]\s*/g, "$1")
      .replace(/^[\s\u200B-\u200D\uFEFF]*i\s+mean\s*[，,、]\s*/gi, "")
      .replace(/([A-Za-z0-9])\s*[，,、]\s*i\s+mean\s*[，,、]\s*(?=[A-Za-z])/gi, "$1 ")
      .replace(/([，,、])\s*i\s+mean(?=$|[\s]*[.。!?！？])/gi, "$1")
      .replace(/^[\s\u200B-\u200D\uFEFF]*actually\s*[，,、]\s*/gi, "")
      .replace(/([A-Za-z0-9])\s*[，,、]\s*actually\s*[，,、]\s*(?=[A-Za-z])/gi, "$1 ")
      .replace(/[，,、]\s*actually\s*([.。!?！？])/gi, "$1")
      .replace(/[，,、]\s*actually(?=$|[\n])/gi, "")
      .replace(/^[\s\u200B-\u200D\uFEFF]*honestly\s*[，,、]\s*/gi, "")
      .replace(/([A-Za-z0-9])\s*[，,、]\s*honestly\s*[，,、]\s*(?=[A-Za-z])/gi, "$1 ")
      .replace(/[，,、]\s*honestly\s*([.。!?！？])/gi, "$1")
      .replace(/[，,、]\s*honestly(?=$|[\n])/gi, "")
      .replace(/^[\s\u200B-\u200D\uFEFF]*frankly\s*[，,、]\s*/gi, "")
      .replace(/([A-Za-z0-9])\s*[，,、]\s*frankly\s*[，,、]\s*(?=[A-Za-z])/gi, "$1 ")
      .replace(/[，,、]\s*frankly\s*([.。!?！？])/gi, "$1")
      .replace(/[，,、]\s*frankly(?=$|[\n])/gi, "")
