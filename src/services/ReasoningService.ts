...
const PRESERVED_UPPERCASE_FILLER_TOKENS = new Set(["UH", "UM"]);
...
- If the source is short/ambiguous, only minimally clean it; do not transform it into assistant dialogue.`;
  ...
  private isAnswerLikeOutput(text: string): boolean {
    ...
    const patterns = [
      ...
      /\b(please tell me)\b/i,
      /\bdon't worry(?:\s+about\s+(?:that|this|it)\b)?(?:\s*[，,.!?。！？:;]\s*|\s+)(?:i(?:'ll| will| can)\s+(?:help|assist)|we(?:'ll| will| can)\s+(?:help|assist)|let me\s+(?:help|assist)|i(?:'m| am)\s+here\s+to\s+help|how can i assist)\b/i,
      /\b(i can help(?:\s+you)?\s+with\s+(?:that|this|it|your\b))\b/i,
      /\b(what can i\s+(?:help(?:\s+you)?(?:\s+with)?|assist(?:\s+you)?(?:\s+with)?|do\s+for\s+you))\b/i,
  ...
  private localCleanupFallback(text: string): string {
    return text
      ...
      .replace(/\b(?:um+|uh+|er+|ah+|hmm+|mm+|you\s+know|basically)\b/gi, (match) => {
        const compact = (match || "").replace(/\s+/g, "");
        if (PRESERVED_UPPERCASE_FILLER_TOKENS.has(compact)) {
          return match;
        }
        return "";
      })
