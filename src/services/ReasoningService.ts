...
- If the source is short/ambiguous, only minimally clean it; do not transform it into assistant dialogue.`;
  ...
  private isQuestionLikeText(text: string): boolean {
    ...
    const zhQuestionPatterns = [
      /[吗么呢吧]$/,
      /^(?:(?:请问|想问(?:下|一下)?|我想问(?:下|一下)?|那|那么|所以|然后)\s*)*(?:什么|谁|哪(?:里|儿)?|为什么|为何|怎么|怎样|几时|几点|多少|是否|几(?:个|位|名|条|项|份|台|次|句|行|段|年|月|周|天|日|号|点|分|秒|时|钟|级|本|件|篇|集|层|届|期|套|辆|岁))/,
      /(?:是|在|叫|有)\s*(?:什么|谁|哪(?:里|儿)?|几时|几点|多少|是否|几(?:个|位|名|条|项|份|台|次|句|行|段|年|月|周|天|日|号|点|分|秒|时|钟|级|本|件|篇|集|层|届|期|套|辆|岁))(?:[\u4e00-\u9fff]{0,4})$/,
