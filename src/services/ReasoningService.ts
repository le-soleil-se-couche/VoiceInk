...
- If the source is short/ambiguous, only minimally clean it; do not transform it into assistant dialogue.`;
  ...
  private isAnswerLikeOutput(text: string): boolean {
    ...
    const patterns = [
      ...
      /\bhere(?:'s| is)\s+(?:the\s+)?(?:cleaned(?:-up)?|polished|revised|edited)\s+(?:version|text|copy)\b/i,
      /^(?:cleaned(?:-up)?|polished|revised|edited)\s+(?:version|text|copy)[\s:]/i,
      /^below\s+is\s+(?:the\s+)?(?:cleaned(?:-up)?|polished|revised|edited)\s+(?:version|text|copy)[\s:]/i,
  ...
  private isQuestionLikeText(text: string): boolean {
    ...
    const zhQuestionEndingPatterns = [
      ...
      /(?:行|对|成|好|要|改|做|发|去|来|上|下|过|给|删|加|留|推|开|关|用|走|写|传|试|买|卖|查|排|定|跟|等|讲|说|看|提|升|降|回|进|出|跑|停|换|调|配|装|拆|补|修)不[啊呀呢嘛]*$/,
    ];
    ...
    const enQuestionStart =
      /^(?:what|when|where|why|who|whom|whose|which|how|is|are|am|was|were|do|does|did|can|could|would|should|shall|will|have|has|had|may|might|must)\b/;
    ...
    const enTagQuestionEnd =
      /(?:,\s*|\s+)(?:isn['’]?t|aren['’]?t|wasn['’]?t|weren['’]?t|don['’]?t|doesn['’]?t|didn['’]?t|can['’]?t|couldn['’]?t|wouldn['’]?t|shouldn['’]?t|won['’]?t|haven['’]?t|hasn['’]?t|hadn['’]?t|mustn['’]?t|is|are|was|were|do|does|did|can|could|would|should|will|have|has|had|must)(?:n['’]?t)?\s+(?:i|you|we|he|she|it|they|there)\s*$/;
  ...
  private isQuestionAnswerRewrite(source: string, candidate: string): boolean {
    ...
    const leadingQuestionAuxiliary =
      normalizedSourceWords.match(
        /^(is|are|am|was|were|do|does|did|can|could|would|should|shall|will|have|has|had|may|might|must)\b/
      )?.[1] || null;
