...
const shouldPreserveLexicalBasically = (input: string, offset: number, matchLength: number): boolean => {
  const before = input.slice(0, offset);
  if (!COPULAR_ENGLISH_VERB_RE.test(before)) {
    return false;
  }

  const after = input.slice(offset + matchLength);
  if (/^\s*[,，、]/.test(after)) {
    return false;
  }
...
const shouldPreserveLexicalUmUnit = (input: string, offset: number, match: string): boolean => {
  if (!/^uM$/.test(match)) {
    return false;
  }
