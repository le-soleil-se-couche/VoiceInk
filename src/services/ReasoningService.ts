...
const stripEnglishFillerMatch = (match: string, offset: number, input: string): string => {
...
- If the source is short/ambiguous, only minimally clean it; do not transform it into assistant dialogue.`;
  ...
  private isAnswerLikeOutput(text: string): boolean {
    ...
    const patterns = [
      ...
      /(?:对不起|抱歉)[，,、\s]*(?:我会|我将|让我(?:来)?|我们(?:会|可以|来)?).{0,20}(?:帮(?:助)?|协助|处理|回答|解释|说明|提供)/,
      ...
      /^(?:sure|yes|yeah|yep|okay|ok|alright|certainly|of\s+course|absolutely)[,，]\s+(?:what|when|where|why|who|which|how|is|are|am|do|does|did|can|could|would|should|will|has|have|had|need)\b/i,
  ...
  private isQuestionLikeText(text: string): boolean {
    ...
    const enQuestionStart =
      /^(?:what|when|where|why|who|whom|whose|which|how|is|are|am|was|were|do|does|did|can|could|would|should|will|have|has|had|may|need)\b/;
