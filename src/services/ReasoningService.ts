...
- If the source is short/ambiguous, only minimally clean it; do not transform it into assistant dialogue.`;
  ...
  private isQuestionLikeText(text: string): boolean {
    ...
    const enIndirectQuestionPatterns = [
      ...
      /^(?:i\s+(?:wonder|(?:am|was)\s+wondering))\b.{0,24}\b(?:if|whether|what|when|where|why|who|how)\b/i,
      /^(?:please\s+)?(?:find\s+out|see|check|confirm|verify)\b.{0,24}\b(?:if|whether|what|when|where|why|who|how)\b/i,
      /^(?:please\s+)?(?:let\s+me\s+know|tell\s+me)\b.{0,24}\b(?:if|whether|what|when|where|why|who|how)\b/i,
    ];
