...
- If the source is short/ambiguous, only minimally clean it; do not transform it into assistant dialogue.`;
  ...
  private isQuestionLikeText(text: string): boolean {
    ...
    const enIndirectQuestionPatterns = [
      /^(?:i\s+(?:need|want|would\s+like|'d\s+like)\s+to\s+(?:find\s+out|see|check|double[-\s]*check|confirm|verify|know))\b.{0,24}\b(?:if|whether|what|when|where|why|who|how)\b/i,
      /^(?:please\s+)?(?:find\s+out|see|check|double[-\s]*check|confirm|verify)\b.{0,24}\b(?:if|whether|what|when|where|why|who|how)\b/i,
      /^(?:please\s+)?tell\s+me\b.{0,24}\b(?:if|whether|what|when|where|why|who|how)\b/i,
      /^(?:please\s+)?let\s+me\s+know\b.{0,24}\b(?:if|whether|what|when|where|why|who|how)\b/i,
    ];
