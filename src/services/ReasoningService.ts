...
- If the source is short/ambiguous, only minimally clean it; do not transform it into assistant dialogue.`;
  ...
  private isAnswerLikeOutput(text: string): boolean {
    ...
    const patterns = [
      ...
      /\b(i\s*(?:can't|cannot)\s+help(?:\s+with)?(?:\s+that)?(?:\s+(?:request|question|task))?)\b/i,
      /\b(i\s+am\s+unable\s+to\s+(?:help|provide)(?:\s+that)?(?:\s+(?:information|request|assistance|support))?)\b/i,
      /\b(i\s+won't\s+do\s+that)\b/i,
