...
- If the source is short/ambiguous, only minimally clean it; do not transform it into assistant dialogue.`;
  ...
  private isAnswerLikeOutput(text: string): boolean {
    ...
    const patterns = [
      ...
      /^(?:sure|yes|yeah|yep|okay|ok|alright|certainly|of\s+course|absolutely)\s*(?:[,，.!?。！？、]|[-–—:])\s*(?:what|when|where|why|who|which|how|is|are|am|do|does|did|can|could|would|should|will|has|have|had)\b/i,
  ...
  private isQuestionLikeText(text: string): boolean {
    ...
    const enQuestionEnd =
      /\b(?:or\s+not|(?:yes|no)\s+or\s+(?:yes|no)|right|correct|okay|ok)\s*$/;
    if (enQuestionEnd.test(normalized)) {
      return true;
    }

    const enIndirectQuestionPatterns = [
      /^(?:i\s+(?:need|want|would\s+like|'d\s+like)\s+to\s+(?:find\s+out|figure\s+out|see|check|confirm|know))\b.{0,24}\b(?:if|whether|what|when|where|why|who|how)\b/i,
      /^(?:i\s+(?:wonder|am\s+wondering|was\s+wondering))\b.{0,24}\b(?:if|whether|what|when|where|why|who|how)\b/i,
      /^(?:please\s+)?(?:find\s+out|figure\s+out|see|check|confirm|tell\s+me|let\s+me\s+know)\b.{0,24}\b(?:if|whether|what|when|where|why|who|how)\b/i,
    ];
