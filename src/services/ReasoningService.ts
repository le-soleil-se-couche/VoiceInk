...
- If the source is short/ambiguous, only minimally clean it; do not transform it into assistant dialogue.`;
  ...
  private isAnswerLikeOutput(text: string): boolean {
    ...
    const patterns = [
      ...
      /(不用担心|别担心|我会尽力|我可以帮你|[你您]想要).{0,40}/,
      /(请告诉我|请问你).{0,24}(?:测试|试试|尝试).{0,30}(?:语音转文字|转录|句子|示例)/,
      ...
      /\b(i\s*(?:can't|cannot|am unable|won't)\s+(?:to\s+)?(?:help|assist|provide|answer|comply|fulfill)\b[^.!?\n]{0,24}\b(?:that|this|request|question|information)\b)/i,
      /\b(i\s*(?:can't|cannot|am unable|won't)\s+do\s+(?:that|this))\b/i,
