...
- If the source is short/ambiguous, only minimally clean it; do not transform it into assistant dialogue.`;
  ...
  private isAnswerLikeOutput(text: string): boolean {
    ...
    const patterns = [
      ...
      /(不用担心|别担心|我会尽力|我可以帮你).{0,40}/,
      /(请告诉我|请问你).{0,24}(?:想要|需要|希望|测试|试试|尝试|更多|具体|怎么|如何|吗|麼|么)/,
      ...
      /^\s*i(?:'m| am)\s+here\s+to\s+help(?:\s+you)?(?:\s+with\s+that)?\s*[,.!?，。！？]\s*(?:(?:what|which|who|when|where|why|how)\b.*(?:[?？]\s*)?|(?:is|are|am|was|were|do|does|did|can|could|would|should|must|will|has|have|had)\s+(?:i|we|you|he|she|it|they|this|that|these|those|there|the|a|an|my|your|our|their|his|her|its)\b.*(?:[?？]\s*)?)$/i,
