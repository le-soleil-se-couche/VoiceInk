...
const HYPHENATED_UH_LEXICAL_SUFFIX_RE = /^\s*[-‐‑–—]\s*(?:oh|huh)\b/i;
const HYPHENATED_MM_HMM_SUFFIX_RE = /^\s*[-‐‑–—]\s*hmm+\b/i;
const HYPHENATED_MM_HMM_PREFIX_RE = /\bmm+\s*[-‐‑–—]\s*$/i;
...
- If the source is short/ambiguous, only minimally clean it; do not transform it into assistant dialogue.`;
  ...
  private isAnswerLikeOutput(text: string): boolean {
    ...
    const patterns = [
      ...
      /^(?:sure|yes|yeah|yep|okay|ok|alright|certainly|of\s+course|absolutely)[,，]\s+(?:what|when|where|why|who|which|how|is|are|am|do|does|did|can|could|would|should|will|has|have|had|might)\b/i,
  ...
  private isQuestionLikeText(text: string): boolean {
    ...
    const enQuestionStart =
      /^(?:what|when|where|why|who|whom|whose|which|how|is|are|am|was|were|do|does|did|can|could|would|should|will|have|has|had|may|might)\b/;
  ...
  private localCleanupFallback(text: string): string {
    return text
      ...
      .replace(ENGLISH_FILLER_WORD_RE, (match, offset, sourceText) => {
        ...
        const beforeMatch = sourceText.slice(0, offset);
        const afterMatch = sourceText.slice(offset + match.length);
        if (
          compactMatch === "mm" &&
          (NUMERIC_MILLIMETER_CONTEXT_RE.test(beforeMatch) ||
            HYPHENATED_MM_HMM_SUFFIX_RE.test(afterMatch))
        ) {
          return match;
        }
        if (compactMatch.startsWith("hmm") && HYPHENATED_MM_HMM_PREFIX_RE.test(beforeMatch)) {
          return match;
        }
        if (
          compactMatch === "uh" &&
          HYPHENATED_UH_LEXICAL_SUFFIX_RE.test(afterMatch)
        ) {
          return match;
        }
        return "";
