...
describe("ReasoningService strict mode", () => {
  ...
  it("falls back when a Chinese which-question is rewritten into a declarative statement", async () => {
    const source = "哪个版本能跑";
    const candidate = "这个版本能跑。";
  ...
  it("keeps Chinese arithmetic cleanup output when the question intent is preserved", async () => {
    const source = "5+5等于几";
    const candidate = "5 + 5 等于几？";
  ...
  it("falls back when a question dictation keeps the question text and appends an inline answer", async () => {
    const source = "what is the capital of france";
  ...
  it("falls back when a question dictation is followed by a same-sentence comma answer", async () => {
    const source = "what is the capital of france";
    const candidate = "What is the capital of France, the capital of France is Paris.";
  ...
  it("falls back when a question dictation is followed by an answer after question punctuation without spacing", async () => {
    const source = "what is the capital of france";
    const candidate = "What is the capital of France?Paris.";
  ...
  it("falls back when assistant-style wrapper omits punctuation before an English question", async () => {
    const source = "what is the capital of france";
    const candidate = "Sure what is the capital of France?";
  ...
  it("falls back when Chinese assistant wrapper omits punctuation before a question", async () => {
    const source = "这个要改吗";
