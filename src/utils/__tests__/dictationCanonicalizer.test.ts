...
const CASES: CanonCase[] = [
  ...
  { name: "时间前缀凌晨一点点保持", input: "凌晨一点点提醒我", expected: "凌晨一点点提醒我" },
  { name: "时间前缀凌晨一点一点保持", input: "凌晨一点一点提醒我", expected: "凌晨一点一点提醒我" },
  { name: "时间前缀凌晨一点儿一点儿保持", input: "凌晨一点儿一点儿提醒我", expected: "凌晨一点儿一点儿提醒我" },
  { name: "时间前缀凌晨一点都保持口语", input: "凌晨一点都不困", expected: "凌晨一点都不困" },
  { name: "时间前缀今晨一点不保持口语", input: "今晨一点不饿", expected: "今晨一点不饿" },
  { name: "时间前缀今晨一点没保持口语", input: "今晨一点没饿", expected: "今晨一点没饿" },
  { name: "时间前缀今晨一点沒保持口语", input: "今晨一点沒饿", expected: "今晨一点沒饿" },
  { name: "时间前缀今晨一点无保持口语", input: "今晨一点无变化", expected: "今晨一点无变化" },
  { name: "时间前缀今晨一点未保持口语", input: "今晨一点未变", expected: "今晨一点未变" },
  { name: "时间前缀今晨一点并不保持口语", input: "今晨一点并不饿", expected: "今晨一点并不饿" },
  { name: "时间前缀今晨一点并没保持口语", input: "今晨一点并没饿", expected: "今晨一点并没饿" },
  { name: "时间前缀今晨一点并未保持口语", input: "今晨一点并未改变", expected: "今晨一点并未改变" },
  { name: "时间前缀凌晨一点一仍按时刻处理", input: "凌晨一点一开会", expected: "凌晨1点一开会" },
  { name: "时间前缀今晨一点都保持口语", input: "今晨一点都不饿", expected: "今晨一点都不饿" },
  { name: "时间前缀凌晨一点也不保持口语", input: "凌晨一点也不困", expected: "凌晨一点也不困" },
  { name: "时间前缀昨夜一点也没保持口语", input: "昨夜一点也没睡", expected: "昨夜一点也没睡" },
  { name: "时间前缀今晨一点也沒保持口语", input: "今晨一点也沒饿", expected: "今晨一点也沒饿" },
  { name: "时间前缀今晨一点也并不保持口语", input: "今晨一点也并不饿", expected: "今晨一点也并不饿" },
  { name: "时间前缀今晨一点也并没保持口语", input: "今晨一点也并没饿", expected: "今晨一点也并没饿" },
  { name: "时间前缀今晨一点也並未保持口语", input: "今晨一点也並未改变", expected: "今晨一点也並未改变" },
  { name: "时间前缀今晨一点逗号也不保持口语", input: "今晨一点，也不饿", expected: "今晨一点，也不饿" },
  { name: "时间前缀今晨一点逗号并不保持口语", input: "今晨一点，并不饿", expected: "今晨一点，并不饿" },
  { name: "时间前缀今晨一点问题都保持口语", input: "今晨一点问题都没有", expected: "今晨一点问题都没有" },
  { name: "时间前缀今晨一点问题也保持口语", input: "今晨一点问题也没有", expected: "今晨一点问题也没有" },
  { name: "时间前缀今晨一点逗号问题都保持口语", input: "今晨一点，问题都没有", expected: "今晨一点，问题都没有" },
  { name: "时间前缀今晨一点儿问题都保持口语", input: "今晨一点儿问题都没有", expected: "今晨一点儿问题都没有" },
  { name: "时间前缀今晨一点儿逗号问题都保持口语", input: "今晨一点儿，问题都没有", expected: "今晨一点儿，问题都没有" },
  { name: "时间前缀今晨一点兒問題都保持口语", input: "今晨一点兒問題都沒有", expected: "今晨一点兒問題都沒有" },
  { name: "时间前缀今晨一点兒逗号問題都保持口语", input: "今晨一点兒，問題都沒有", expected: "今晨一点兒，問題都沒有" },
  { name: "时间前缀凌晨一点儿都保持口语", input: "凌晨一点儿都不困", expected: "凌晨一点儿都不困" },
  { name: "时间前缀凌晨一点兒都保持口语", input: "凌晨一点兒都不困", expected: "凌晨一点兒都不困" },
  { name: "时间前缀今晨一点儿也保持口语", input: "今晨一点儿也不饿", expected: "今晨一点儿也不饿" },
  { name: "时间前缀今晨一点兒也保持口语", input: "今晨一点兒也不饿", expected: "今晨一点兒也不饿" },
  { name: "时间前缀今晨一点儿不保持口语", input: "今晨一点儿不饿", expected: "今晨一点儿不饿" },
  { name: "时间前缀今晨一点兒不保持口语", input: "今晨一点兒不饿", expected: "今晨一点兒不饿" },
  { name: "时间前缀今晨一点兒沒保持口语", input: "今晨一点兒沒饿", expected: "今晨一点兒沒饿" },
  { name: "时间前缀今晨一点儿并不保持口语", input: "今晨一点儿并不饿", expected: "今晨一点儿并不饿" },
  { name: "时间前缀今晨一点兒並沒保持口语", input: "今晨一点兒並沒饿", expected: "今晨一点兒並沒饿" },
  { name: "时间前缀凌晨一点兒一点保持", input: "凌晨一点兒一点提醒我", expected: "凌晨一点兒一点提醒我" },
  {
    name: "时间前缀重复一点一刻保持口语",
    input: "凌晨一点一点一刻走",
    expected: "凌晨一点一点一刻走",
  ...
  {
    name: "时间前缀重复一点儿十五分保持口语",
    input: "凌晨一点儿一点十五分走",
    expected: "凌晨一点儿一点十五分走",
  },
  {
    name: "时间前缀重复一点兒一刻保持口语",
    input: "凌晨一点兒一点一刻走",
    expected: "凌晨一点兒一点一刻走",
  },
  { name: "时间前缀凌晨一点一刻仍按时刻处理", input: "凌晨一点一刻走", expected: "凌晨1点一刻走" },
  { name: "时间前缀凌晨一点也要仍按时刻处理", input: "凌晨一点也要开会", expected: "凌晨1点也要开会" },
  { name: "时间前缀凌晨一点逗号也要仍按时刻处理", input: "凌晨一点，也要开会", expected: "凌晨1点，也要开会" },
  { name: "时间前缀凌晨一点也并要仍按时刻处理", input: "凌晨一点也并要开会", expected: "凌晨1点也并要开会" },
  { name: "时间前缀凌晨一点并要仍按时刻处理", input: "凌晨一点并要开会", expected: "凌晨1点并要开会" },
  { name: "时间前缀凌晨一点问题都要仍按时刻处理", input: "凌晨一点问题都要同步", expected: "凌晨1点问题都要同步" },
  { name: "时间前缀凌晨一点逗号问题都要仍按时刻处理", input: "凌晨一点，问题都要同步", expected: "凌晨1点，问题都要同步" },
  { name: "时间前缀凌晨一点不要仍按时刻处理", input: "凌晨一点不要迟到", expected: "凌晨1点不要迟到" },
  { name: "时间前缀凌晨一点逗号不要仍按时刻处理", input: "凌晨一点，不要迟到", expected: "凌晨1点，不要迟到" },
  { name: "时间前缀凌晨一点儿仍按时刻处理", input: "凌晨一点儿开会", expected: "凌晨1点儿开会" },
  { name: "时间前缀凌晨一点兒仍按时刻处理", input: "凌晨一点兒开会", expected: "凌晨1点兒开会" },
  { name: "时间前缀凌晨一点仍按时刻处理", input: "凌晨一点开会", expected: "凌晨1点开会" },
  { name: "时间晚一点半仍按时刻处理", input: "晚一点半开会", expected: "晚1点半开会" },
  { name: "口语重复一点保持", input: "一点一点", expected: "一点一点" },
  { name: "句中口语重复一点儿保持", input: "慢慢来一点儿一点儿做", expected: "慢慢来一点儿一点儿做" },
  { name: "口语一点一滴保持", input: "一点一滴", expected: "一点一滴" },
  { name: "固定表达一时半点保持", input: "一时半点", expected: "一时半点" },
  { name: "句中一时半点保持", input: "这事一时半点讲不完", expected: "这事一时半点讲不完" },
  { name: "固定表达一分半点保持", input: "一分半点", expected: "一分半点" },
  { name: "句中一分半点保持", input: "这点内容一分半点不懂", expected: "这点内容一分半点不懂" },
  { name: "固定表达一点半点保持", input: "一点半点", expected: "一点半点" },
  { name: "固定表达一点半点儿保持", input: "一点半点儿", expected: "一点半点儿" },
  { name: "时间前缀一点半点保持口语", input: "凌晨一点半点都不困", expected: "凌晨一点半点都不困" },
