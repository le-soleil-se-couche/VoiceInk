...
const PUNCT_WORD_CONTRASTIVE_COORDINATION_RE =
  /(?:句号|逗号|问号|感叹号|冒号|分号|顿号)\s*还是\s*$/;
// Include unit chars on both sides so phrases like "十一点二" are parsed as one decimal segment
// instead of partially matching "一点二" and corrupting output.
const DECIMAL_SPOKEN_RE =
  /([零〇一二两三四五六七八九十百千万萬\d]+)点([零〇一二两三四五六七八九十百千万萬\d]+)/g;
...
const protectLiteralMentions = (
  ...
  const literalPatterns = [
    ...
    new RegExp(
      `(?:这个(?:标点|符号|词|字)|该(?:标点|符号|词|字))\\s*名为\\s*(?:“)?(?:${termGroup})(?:”)?`,
      "g"
    ),
    new RegExp(
      `(?:这个(?:标点|符号|词|字)|该(?:标点|符号|词|字))(?:\\s*被)?\\s*称之为\\s*(?:“)?(?:${termGroup})(?:”)?`,
      "g"
    ),
...
const applyLowAmbiguityPunctuationRules = (
  ...
  let next = value.replace(SENTENCE_END_PUNCT_WORD_RE, (match, _punctWord, offset, sourceText) => {
    const safeOffset = typeof offset === "number" ? offset : 0;
    const prefix = sourceText.slice(0, safeOffset);
    if (PUNCT_WORD_CONTRASTIVE_COORDINATION_RE.test(prefix)) {
      return match;
    }
